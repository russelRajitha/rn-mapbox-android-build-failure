...
import {ImageLoadEvent, ImageSourcePropType} from 'react-native/Libraries/Image/Image';
...
interface ImagesZoomableProps {
    images: ImageSourcePropType[];
    onChangeIndex?: (event:number)=>void;
    style?: StyleProp<ViewStyle>;
}

const ImagesZoomable = forwardRef<{
    setIndex: (i:number)=>void
}, ImagesZoomableProps>(({images, style, onChangeIndex}, ref) => {
    const [currentIndex, setCurrentIndex] = useState(0)
    const setIndex= useCallback((idx:number) => {
        const newValue = Math.min(Math.max(idx, 0), images.length - 1)
    ...
    const handleSwipePage = useCallback((direction: 'next' | 'prev') => {
        setCurrentIndex(prev => {
            let newValue = direction === 'next' ? Math.min(prev + 1, images.length - 1):Math.max(prev - 1, 0)
            onChangeIndex?.(newValue)
            return newValue
        })
    }, [])
    ...
    const panOnEnd = useCallback((event: GestureUpdateEvent<PanGestureHandlerEventPayload>) => {
        ...
        if (scale.value <= MinScale) {
            ...
            const shouldSwipeNext = (dx < -SWIPE_DISTANCE || vx < -SWIPE_VELOCITY)
            const shouldSwipePrev = (dx > SWIPE_DISTANCE || vx > SWIPE_VELOCITY)

            if (shouldSwipeNext) {
                scheduleOnRN(handleSwipePage, 'next')
            } else if (shouldSwipePrev) {
                scheduleOnRN(handleSwipePage, 'prev')
            }
            translationX.value = 0
            translationY.value = 0
            return
    ...
    const sizeStyle = useAnimatedStyle(() => ({
        ...
        height: Math.min((imageHeight.value / imageWidth.value) * containerWidth.value,containerHeight.value),
    }))
